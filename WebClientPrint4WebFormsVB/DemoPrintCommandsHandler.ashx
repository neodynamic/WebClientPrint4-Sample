    ...
    Private Sub ClientPrint(sid As String)
        If WebClientPrint.ProcessPrintJob(System.Web.HttpContext.Current.Request.Url.Query) Then
            ...
            ElseIf printerTypeId = 5 Then
                ...
                cpj.ClientPrinter = New SerialPortPrinter(app(sid & SERIAL_PORT).ToString(), Integer.Parse(app(sid & SERIAL_PORT_BAUDS).ToString()), CType([Enum].Parse(GetType(System.IO.Ports.Parity), app(sid & SERIAL_PORT_PARITY).ToString()), System.IO.Ports.Parity), CType([Enum].Parse(GetType(System.IO.Ports.StopBits), app(sid & SERIAL_PORT_STOP_BITS).ToString()), System.IO.Ports.StopBits), Integer.Parse(app(sid & SERIAL_PORT_DATA_BITS).ToString()), CType([Enum].Parse(GetType(System.IO.Ports.Handshake), app(sid & SERIAL_PORT_FLOW_CONTROL).ToString()), System.IO.Ports.Handshake))
            End If
